...
export function initDb() {
  db.exec(`
    ...
    CREATE TABLE IF NOT EXISTS donations (
      ...
      category TEXT NOT NULL DEFAULT 'Food', -- Food, Clothes, Money, Medical Supplies, Others
      food_type TEXT NOT NULL, -- Title of the item
      description TEXT,
      quantity_lbs REAL, -- For food
      expiry_time TEXT, -- For food
      status TEXT NOT NULL DEFAULT 'Pending' CHECK(status IN ('Pending', 'Accepted', 'In Progress', 'In Transit', 'Delivered', 'Completed')),
      pickup_address TEXT,
      pickup_lat REAL,
      pickup_lng REAL,
      food_image_url TEXT,
      ...
      details TEXT, -- JSON string for category-specific fields
    ...
    CREATE TABLE IF NOT EXISTS assignments (
      ...
      status TEXT NOT NULL DEFAULT 'Assigned' CHECK(status IN ('Assigned', 'Picked Up', 'Delivered', 'Completed')),
  ...
  if (count.count === 0) {
    ...
    const insertDonation = db.prepare(`
        INSERT INTO donations (
            donor_id, category, food_type, description, quantity_lbs, expiry_time, status, 
            pickup_address, pickup_lat, pickup_lng, allergens, is_veg, prep_type, 
            storage_type, safety_confirmed, details
        ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
    insertDonation.run(
        1, 'Food', '50 Loaves of Bread', 'Assorted sourdough and wheat bread', 50, 
        new Date(Date.now() + 86400000).toISOString(), 'Pending', 
        '123 Baker St, NY 10001', 40.7128, -74.0060, 'Gluten', 1, 'Restaurant', 
        'Room temperature', 1, JSON.stringify({ packaging_type: 'Paper Bag', allergens: 'Gluten' })
    );
  }
}
